async function getBingDict(text, axios) {
  ...
  function handleExamples(sentence) {
    let regex1 = /\{(\d+)#(.*?)\$.*?\}/g;
...
async function getDictText(text, from, to, axios, t, config) {
  let res = await getBingDict(text, axios);
  if (!res.explanations) {
    return t("Error happened")
  }
  window.learnMoreUrl = "https://www.bing.com/dict/search?q=" + text;
  return (
